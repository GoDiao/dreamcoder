...
SESSION_NAME="dreamcoder-context-live-${RUN_ID}"
ARTIFACT_DIR="${ARTIFACT_DIR:-$(mktemp -d "/tmp/dreamcoder-context-live-${RUN_ID}-XXXX")}"
...
"${AB[@]}" eval "localStorage.setItem('dreamcoder-locale', 'en'); localStorage.setItem('dreamcoder-open-tabs', '${TAB_STATE}'); location.reload();" >>"${BROWSER_LOG}" 2>&1
