...
ARTIFACT_DIR="${ARTIFACT_DIR:-/tmp/dreamcoder-e2e-parallel-providers-$$}"
CONFIG_DIR="${ARTIFACT_DIR}/config"
PROJECT_A="${ARTIFACT_DIR}/project-a"
PROJECT_B="${ARTIFACT_DIR}/project-b"
SESSION_NAME_A="${SESSION_NAME_A:-dreamcoder-provider-a-$$}"
SESSION_NAME_B="${SESSION_NAME_B:-dreamcoder-provider-b-$$}"
...
mkdir -p "${ARTIFACT_DIR}" "${CONFIG_DIR}/dreamcoder" "${PROJECT_A}" "${PROJECT_B}"
...
mkdirSync(join(configDir, 'dreamcoder'), { recursive: true })
writeFileSync(join(configDir, 'dreamcoder', 'providers.json'), JSON.stringify({
  schemaVersion: 1,
...
setup_browser() {
  ...
  "${ab[@]}" eval "
    localStorage.setItem('dreamcoder-open-tabs', JSON.stringify({
      openTabs: [{ sessionId: '${session_id}', title: '${title}', type: 'session' }],
      activeTabId: '${session_id}',
    }));
    localStorage.setItem('dreamcoder-session-runtime', JSON.stringify({
